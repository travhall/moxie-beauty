...
function ServiceRow({
  ...
  return (
    ...
    >
      {/* Service number */}
      <span
        className="font-mono text-[11px] tracking-widest text-(--ink-mute) pt-1.5 shrink-0 tabular-nums"
        aria-hidden="true"
      >
        {num}
      </span>
      ...
      <div className="flex-1 min-w-0">
        {badge && (
          <span className="inline-block mb-2.5 px-2.5 py-1 rounded-full bg-(--accent)/15 text-(--accent) text-[10px] font-nyght-bold tracking-[0.15em] uppercase">
            {badge}
          </span>
        )}
